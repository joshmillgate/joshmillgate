...
**IMPORTANT:** NEVER use browser MCP tools to check or verify the browser. The user will verify browser changes themselves.
...
**IMPORTANT:** When using animated Lottie JSON icons, always ensure they have light/dark mode compatibility by styling the SVG paths with appropriate stroke colors. Use the pattern:
```css
.icon-container svg path {
  stroke: #B2B2B2; /* Light mode color */
}

@media (prefers-color-scheme: dark) {
  .icon-container svg path {
    stroke: #666; /* Dark mode color */
  }
}
```
This ensures icons remain visible and properly styled in both light and dark themes.
